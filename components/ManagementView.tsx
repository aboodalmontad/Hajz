...
const ManagementView: React.FC = () => {
    // Hooks and state
    const { clerks, addClerk, removeClerk, customers, servedCount, windows, addWindow, removeWindow } = useQueue();
    // Clerk form state
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
            <h1 className="text-3xl font-bold text-gray-800">الإدارة والتحكم</h1>
            ...
            <div className="space-y-8">
                <h2 className="text-2xl font-bold text-gray-800">إدارة النوافذ</h2>
                 <div className="bg-white p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-semibold mb-4">إضافة نافذة جديدة</h3>
                    <form onSubmit={handleAddWindow} className="flex items-end gap-4">
                        <div className="flex-grow">
                            <label htmlFor="windowNumberInput" className="block text-sm font-medium text-gray-700">رقم النافذة</label>
                            <input
                                type="number" id="windowNumberInput" value={newWindowNumber} onChange={(e) => setNewWindowNumber(e.target.value)}
                                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                                placeholder="مثال: 10" required
                            />
                        </div>
                        <button type="submit" className="bg-primary-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-primary-700 transition">
                            إضافة نافذة
                        </button>
                    </form>
                </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-semibold mb-4">النوافذ الحالية</h3>
                     <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                             <thead className="bg-gray-50">
                                <tr>
                                    <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">رقم النافذة</th>
                                    <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الموظف المسؤول</th>
                                    <th scope="col" className="relative px-6 py-3"><span className="sr-only">إجراءات</span></th>
                                </tr>
                            </thead>
                             <tbody className="bg-white divide-y divide-gray-200">
                                {windows.map((window) => {
                                    const assignedClerk = clerks.find(c => c.windowId === window.id);
                                    return (
                                    <tr key={window.id}>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{window.number}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{assignedClerk ? assignedClerk.name : <span className="text-green-600">متاح</span>}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                                            <button
                                                onClick={() => removeWindow(window.id)}
                                                disabled={!!assignedClerk}
                                                className="text-red-600 hover:text-red-900 disabled:text-gray-400 disabled:cursor-not-allowed"
                                                title={assignedClerk ? "لا يمكن إزالة نافذة مخصصة لموظف" : "إزالة النافذة"}
                                            >
                                                إزالة
                                            </button>
                                        </td>
            ...
            <div className="space-y-8">
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-semibold mb-4">إضافة موظف جديد</h3>
                    <form onSubmit={handleAddClerk} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        <div>
                            ...
                            <input
                                ...
                                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                                placeholder="مثال: ahmad.ali" required
                            />
                        </div>
                        <div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h3 className="text-xl font-semibold mb-4">الموظفون الحاليون</h3>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">الاسم</th>
                                    <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">اسم المستخدم</th>
                                ...
                                {clerks.map((clerk) => {
                                    ...
                                    return (
                                        <tr key={clerk.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{clerk.username}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{window ? window.number : 'غير مسجل'}</td>
