...
const ClerkCard: React.FC<{
  ...
  onLogout: () => void;
}> = ({ clerk, onCallNext, onFinishService, onSetStatus, customer, onLogout }) => {
  const { windows } = useQueue();
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-gray-500">شباك {window ? window.number : 'غير معين'}</p>
          </div>
      ...
      <div className="mt-6 space-y-2">
        ...
        <button
          onClick={onLogout}
          className="w-full bg-red-200 text-red-800 font-bold py-2 px-4 rounded-lg hover:bg-red-300 transition"
        >
          تسجيل الخروج
        </button>
      </div>
...
const ClerkView: React.FC<ClerkViewProps> = ({ clerk, onLogout }) => {
    const { callNextCustomer, finishService, setClerkStatus, serving } = useQueue();
    ...
    return (
        <div className="container mx-auto flex flex-col items-center pt-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">لوحة التحكم الخاصة بك</h1>
